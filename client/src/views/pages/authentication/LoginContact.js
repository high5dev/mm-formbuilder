...
import { contactsAction } from '@src/views/contacts/store/actions';
import { getTagsAction, getStagesAction, getLeadsSourceAction } from '../../contacts/store/actions';
...
const LoginContact = () => {
  // ** Hooks
  const { contactTypeId, assignerId } = useParams();
  ...
  const { contactLoginRequest } = useAuth();
  ...
  const fetchData = async () => {
    // ** Fetching Contact List & Contact Types
    ...
    if (Object.values(data).every((field) => field.length > 0)) {
      ...
      contactLoginRequest({
        ...
        .then((res) => {
          ...
          if (res.userData.isEmployee) {
            newAbility.push(
              // {
              //   action: 'read',
              //   subject: 'contacts'
              // },
              { action: 'read', subject: 'contacts/employeeInfo' }
            );
          }
        ...
        <Col className="d-flex align-items-center auth-bg px-2 p-lg-5" lg="4" sm="12">
          <Col className="px-xl-2 mx-auto" sm="8" md="6" lg="12">
            ...
            <Form className="auth-login-form mt-2" onSubmit={handleSubmit(onSubmit)}>
              ...
              <Button disabled={isLoading} type="submit" color="primary" block>
                {isLoading ? 'Authenticating...' : 'Sign in'}
              </Button>
            </Form>
            <p className="text-center mt-2">
              <span className="me-25">New on our platform?</span>
              <Link to={`/register/${contactTypeId}/${assignerId}`}>
