...
const Register = () => {
  ...
  const { sendSignupOtp, getVisitorInfo, orgRegistrationReqeust } = useAuth();
  ...
  const onSubmit = () => {
    ...
    if (isOtpSended) {
      ...
      setIsLoading(true);
      orgRegistrationReqeust(path, data)
        .then((response) => {
          setIsLoading(false);
    ...
    if (Object.values(tempData).every((field) => field.length > 0) && data.terms === true && data.receiveEmails===true) {
      setIsLoading(true);
      sendSignupOtp({ ...data, countryCode, orgSignup: true })
        .then((response) => {
          setIsLoading(false);
          setIsOtpSended(true);
        })
  ...
  useEffect(() => {
    // if(path){
    //     //get organization color & logo & name
    //     //name
    //     //themeColor
    //     //logoLink
    // }
    ...
    <div className="auth-wrapper auth-cover">
      <Row className="auth-inner m-0">
        <Link className="brand-logo" to="/" onClick={(e) => e.preventDefault()}>
          {org && org.logoLink ? (
            <>
              <img src={org.logoLink} height="28" />
            </>
          ) : (
          ...
          <h2 onClick={resetForm} className="brand-text text-primary ms-1">
            {org && org.name ? org.name : ' My Manager'}
          </h2>
        </Link>
